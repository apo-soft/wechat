...
public class NewsService implements MessageService {
	...
	private final MpAccountConfig config;

	public NewsService(MpAccountConfig config) {
		this.config = config;
	}
	...
	public AposoftMessage getReplyMessage(String toUser) {
		...
		// 样例,生产需要转换URL
		article.setUrl("https://www.aposoft.cn/wx/oauth2/authorize?type=" + Oauth2MsgType.ACCESS_TOKEN.getType()
				+ "&scope=" + Oauth2Scope.snsapi_userinfo.getScope());
		...
		// 样例,生产需要转换URL
		article2.setUrl("https://www.aposoft.cn/wx/oauth2/authorize?type=" + Oauth2MsgType.USER_INFO.getType()
				+ "&scope=" + Oauth2Scope.snsapi_userinfo.getScope());
