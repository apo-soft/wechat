...
public class WXBizMsgCrypt {
    ...
    String encrypt(String randomStr, String text) throws AesException {
        ...
        byte[] randomStrBytes = randomStr.getBytes(Lexical.UTF8_CHARSET);
        byte[] textBytes = text.getBytes(Lexical.UTF8_CHARSET);
        byte[] networkBytesOrder = getNetworkBytesOrder(textBytes.length);
        byte[] appidBytes = appId.getBytes(Lexical.UTF8_CHARSET);
    ...
    String decrypt(String text) throws AesException {
        ...
        try {
            ...
            xmlContent = new String(Arrays.copyOfRange(bytes, 20, 20 + xmlLength), Lexical.UTF8_CHARSET);
            from_appid = new String(Arrays.copyOfRange(bytes, 20 + xmlLength, bytes.length), Lexical.UTF8_CHARSET);
        } catch (Exception e) {
            e.printStackTrace();
